...
export default function PaymentSummaryCard({ landlord_id, onClick }: Props) {
  ...
  const options = useMemo(
    () => ({
      chart: {
        type: "donut" as const,
        animations: { enabled: !statsLoading },
      },
  ...
  return (
    <div
      onClick={onClick}
      className={`${CARD_CONTAINER_INTERACTIVE} min-h-[420px] flex flex-col select-none`}
    >
      {/* Header */}
      <div className="flex justify-between items-start mb-6">
        <div className={SECTION_HEADER}>
          <span className={GRADIENT_DOT} />
          <h2 className={SECTION_TITLE}>Payment Summary</h2>
        </div>
...
function Stat({
  ...
  return (
    <div
      className={`p-4 rounded-xl border ${colorMap[color]} flex flex-col items-center shadow-inner`}
    >
