...
const RevenuePerformanceChart = ({ landlord_id }: { landlord_id: number }) => {
    ...
    const containerClasses =
        "bg-white rounded-lg shadow-sm border border-gray-200 p-4 md:p-6 transition-all duration-200 hover:shadow-md cursor-pointer";

    /* 🔥 WRAPPER THAT REDIRECTS ON CLICK */
    const wrapperProps = {
        onClick: () =>
    ...
    /* ▬▬▬ LOADING STATE ▬▬▬ */
    if (loading) {
        return <div {...wrapperProps}>{/* Your skeleton stays the same */}</div>;
    }
