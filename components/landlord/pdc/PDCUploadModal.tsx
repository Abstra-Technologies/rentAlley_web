...
export default function PDCUploadModal({
                                           open,
                                           onClose,
                                           landlord_id,
                                       }: PDCUploadModalProps) {
    ...
    const handleDeleteImage = async () => {
        if (!form.uploaded_image_url) return;

        const confirm = await Swal.fire({
            title: "Delete Image?",
            text: "This will permanently delete the uploaded image from storage.",
            icon: "warning",
        ...
        try {
            Swal.fire({
                title: "Deleting...",
                text: "Removing image from storage...",
                allowOutsideClick: false,
                didOpen: () => Swal.showLoading(),
            });
            ...
            await fetch("/api/ai/landlord/pdcAnalyze/delete", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ image_url: form.uploaded_image_url }),
            });
    ...
    const handleCancel = async () => {
        ...
        if (form.uploaded_image_url) {
            try {
                ...
                console.warn("Failed to auto-delete image on cancel:", err);
            }
        }
        onClose();
    };
                        ...
                        <div>
                            <div className="flex flex-wrap gap-3 items-center">
                                ...
                                >
                                    ...
                                    {showCamera ? "Close Camera" : "Capture from Camera"}
                                </button>
                                ...
                                {form.uploaded_image_url && (
                                    <button
                                        ...
                                        className="flex items-center gap-2 px-4 py-2 bg-red-600 text-white rounded-lg shadow hover:bg-red-700 transition text-sm"
                                    >
