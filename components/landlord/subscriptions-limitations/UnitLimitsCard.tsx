...
export default function UnitLimitsCard({
  ...
  console.log("🔍 UnitLimitsCard DEBUG:", {
  ...
  const planName = subscription.plan_name || subscription.planName;

  console.log("🔍 FINAL PLAN NAME:", planName);
    console.log("🔍 SHOULD SHOW CARD?", planName === "Free Plan");

  if (planName !== "Free Plan") return null;

  const maxUnits = subscription?.limits?.maxUnits ?? 0; // 👈 FIXED: listingLimits → limits
    ...
    >
      ...
      <div className="flex-1">
        <p className="font-semibold text-sm">
          {reachedLimit ? "Unit limit reached" : "Unit capacity available"}
        </p>
