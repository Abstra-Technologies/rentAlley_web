...
function Step({
  ...
  return (
    <button
      ...
      className={`w-full sm:flex-1 rounded-xl border p-3 sm:p-4 text-left transition ${
        styles[status]
      } ${
        disabled
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0">
          ...
          <p className="text-xs opacity-80 truncate sm:whitespace-normal">
            {description}
          </p>
        </div>
      </div>
...
function Connector({ active }: { active: boolean }) {
  return (
    <div className="hidden sm:flex flex-1 items-center">
      <div
        className={`h-[2px] w-full ${active ? "bg-green-400" : "bg-gray-300"}`}
      />
    </div>
  );
}
