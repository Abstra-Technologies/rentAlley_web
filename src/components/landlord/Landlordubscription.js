...
export default function LandlordSubscriptionCurrent({ user_id }) {
    ...
    if (loading) return <div><p>Loading subscription details...</p></div>;
            ...
            {subscriptions.length > 0 ? (
                <table style={{ width: "100%", borderCollapse: "collapse", border: "1px solid #ddd", borderRadius: "8px" }}>
                    ...
                    <tr style={{ backgroundColor: "#f4f4f4", textAlign: "left" }}>
                        <th style={{ padding: "10px", borderBottom: "1px solid #ddd" }}>Plan Name</th>
                        ...
                        <th style={{ padding: "10px", borderBottom: "1px solid #ddd" }}>Amount Paid</th>
                    </tr>
                    </thead>
                    <tbody>
                    {subscriptions.map((subscription, index) => (
                        <tr key={index} style={{ backgroundColor: subscription.status === "active" ? "#e8f5e9" : "white" }}>
                            ...
                            <td style={{ padding: "10px", borderBottom: "1px solid #ddd" }}>{new Date(subscription.end_date).toLocaleDateString()}</td>
                            <td style={{ padding: "10px", borderBottom: "1px solid #ddd" }}>{subscription.payment_status}</td>
                            <td style={{ padding: "10px", borderBottom: "1px solid #ddd" }}>₱{subscription.amount_paid}</td>
                        </tr>
                    ))}
                    </tbody>
                </table>
            ) : (
                <div><p>No subscription history available.</p></div>
            )}
        </div>
    );
}
