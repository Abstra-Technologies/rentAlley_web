...
//       // Check if the property has already reached the max attempts limit
//       const [verificationRows] = await connection.execute(
//         "SELECT attempts, status FROM PropertyVerification WHERE property_id = ?",
//         [Number(property_id)]
//       );
      
//       // Debug the current state
//       console.log("Current verification state:", verificationRows[0] || "No previous verification");
      
//       // If the property exists in the verification table and already has 4 or more attempts
//       if (verificationRows.length > 0 && verificationRows[0].attempts >= 4 && verificationRows[0].status === 'Rejected') {
//         return res.status(403).json({ 
//           error: "Maximum attempts reached", 
//           message: "This property has been rejected and reached the maximum number of verification attempts (4). Please create a new property listing."
//         });
//       }
...
//       // Check if this is a new entry or update and get current attempts
//       let currentAttempts = 1; // Default for new entries
      
//       if (verificationRows.length > 0) {
//         // Only increment the attempts counter if the current status is 'Rejected' and we're resubmitting
//         // If status is something else, like 'Pending' or 'Verified', don't increment
//         if (verificationRows[0].status === 'Rejected') {
//           currentAttempts = verificationRows[0].attempts + 1;
//         } else {
//           currentAttempts = verificationRows[0].attempts;
//         }
//       }
      
//       console.log("Setting attempts to:", currentAttempts);

//       const query =
//         "INSERT INTO PropertyVerification (property_id, occ_permit, mayor_permit, gov_id, indoor_photo, outdoor_photo, status, created_at, updated_at, verified, attempts)\n" +
//           "VALUES (?, ?, ?, ?, ?, ?, 'Pending', NOW(), NOW(), 0, ?)\n" +
//           "ON DUPLICATE KEY UPDATE \n" +
//           "  occ_permit = COALESCE(VALUES(occ_permit), occ_permit), \n" +
//           "  mayor_permit = COALESCE(VALUES(mayor_permit), mayor_permit), \n" +
//           "  gov_id = COALESCE(VALUES(gov_id), gov_id), \n" +
//           "  indoor_photo = COALESCE(VALUES(indoor_photo), indoor_photo), \n" +
//           "  outdoor_photo = COALESCE(VALUES(outdoor_photo), outdoor_photo), \n" +
//           "  status = 'Pending', \n" +
//           "  updated_at = NOW(), \n" +
//           "  attempts = ?;\n";
...
//         attempts: currentAttempts,
//         attemptsRemaining: Math.max(0, 4 - currentAttempts)
//       });
//     } catch (error) {
