  ...
  try {
    ...
    if (prospectiveTenantData.property_id && !prospectiveTenantData.unit_id) {
      [result] = await db.query(
        `SELECT 
            p.property_id, p.property_name, p.property_type, p.amenities, p.landlord_id,
    ...
    // If the tenant is assigned to a specific unit
      ...
      [result] = await db.query(
        `SELECT 
            ...
            p.property_id, p.property_name, p.city, p.street, p.landlord_id,
    ...
    //deryption fo property image data.
