...
export default function ResubmitVerificationPage({ params }) {
    const { property_id } = params; // Extract property_id from dynamic route
    ...
    useEffect(() => {
        if (property_id) {
            fetch(`/api/propertyListing/propListing?property_id=${property_id}`)
                .then((res) => res.json())
                .then((data) => {
                    setProperty(data[0]); // Assuming API returns an array
                    setLoading(false);
                })
                .catch((error) => {
        ...
        <div className="min-h-screen flex items-center justify-center bg-gray-100">
            <div className="max-w-2xl w-full bg-white p-6 rounded-lg shadow-md">
                <h1 className="text-lg font-bold mb-4">
                    Resubmit Property Verification (ID: {property_id})
                </h1>

                {/* Show verification message if rejected */}
                {property?.verification_status === "Rejected" && (
                    <div className="bg-red-100 text-red-700 p-2 rounded mb-4">
                        <strong>Reason:</strong> {property?.admin_message || "No message provided."}
                    </div>
                )}
