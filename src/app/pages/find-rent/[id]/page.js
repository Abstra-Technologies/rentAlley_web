...
import { FaBed, FaRuler, FaCouch, FaMapMarkerAlt, FaBuilding, FaSwimmingPool, FaWifi, FaUserTie, FaRegHeart, FaHeart, FaShareAlt } from "react-icons/fa";
import { BsImageAlt, BsCheckCircleFill } from "react-icons/bs";
import { MdVerified, MdKeyboardArrowRight, MdOutlineApartment } from "react-icons/md";
...
export default function PropertyDetails() {
    ...
    async function fetchPropertyDetails() {
      try {
        ...
        if (data.units && data.units.length > 0) {
          const availableUnit = data.units.find(unit => unit.status !== "occupied");
          if (availableUnit) {
  ...
  const handleUnitSelection = (unitId) => {
    ...
    const unit = property.units.find(u => u.unit_id === unitId);
  ...
  const getAmenityIcon = (amenity) => {
    ...
    if (lowerCaseAmenity.includes('wifi') || lowerCaseAmenity.includes('internet')) return <FaWifi />;
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="flex items-center">
                <h1 className="text-2xl font-bold text-gray-800">{property.property_name}</h1>
            ...
            {property.property_photo.length > 1 && (
              <div className="flex mt-4 space-x-2 overflow-x-auto pb-2">
                {property.property_photo.map((photo, index) => (
                  <div 
                    key={index}
                    className={`relative w-24 h-24 rounded-lg overflow-hidden cursor-pointer transition transform hover:scale-105 ${activeImageIndex === index ? 'ring-2 ring-blue-500' : 'opacity-80'}`}
          ...
          <div className="lg:col-span-2">
            ...
            <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <h3 className="font-medium text-gray-700">Available Units</h3>
                  <p className="text-gray-600">
                    {property.units.filter(unit => unit.status !== "occupied").length} units
                  </p>
                </div>
              </div>
            ...
            <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
              ...
              {property.units.length === 0 ? (
                <div className="text-center py-8 bg-gray-50 rounded-lg">
                  <p className="text-gray-500">No available units at this time</p>
                </div>
              ) : (
                <>
                  {/* Units List */}
                    ...
                    {property.units.map((unit) => (
                      <div 
                        ...
                        className={`border p-4 rounded-lg cursor-pointer transition hover:shadow-md ${
                          selectedUnitId === unit.unit_id ? 'border-blue-500 bg-blue-50' : 'border-gray-200'
                        } ${unit.status === "occupied" ? 'opacity-60' : ''}`}
                        onClick={() => unit.status !== "occupied" && handleUnitSelection(unit.unit_id)}
                      >
                          ...
                          <div className="md:flex-1 md:ml-4">
                            <div className="flex justify-between items-start">
                              ...
                              <div className="text-right">
                                <div className="font-bold text-lg text-blue-600">₱{unit.rent_payment.toLocaleString()}<span className="text-sm text-gray-500">/month</span></div>
                            ...
                            {/* Selection button for available units */}
                            {unit.status !== "occupied" && (
                              <button 
                                className={`mt-3 w-full md:w-auto px-4 py-2 rounded ${
                              ...
                              >
                                {selectedUnitId === unit.unit_id ? "Selected" : "Select Unit"}
                              </button>
                            )}
                          </div>
                        </div>
            ...
            <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
              ...
              {selectedUnit ? (
                <div className="mb-6 p-4 bg-blue-50 rounded-lg">
                  <h3 className="font-medium">Selected Unit</h3>
                  <div className="flex justify-between items-center mt-2">
                    <p className="text-gray-700">{selectedUnit.unit_name}</p>
                    <p className="font-bold text-blue-600">₱{selectedUnit.rent_payment.toLocaleString()}/mo</p>
                  </div>
                </div>
              ) : (
                <div className="mb-6 p-4 bg-gray-50 rounded-lg text-center">
                  <p className="text-gray-500">No unit selected</p>
                  <p className="text-sm text-gray-400 mt-1">Select a unit to proceed</p>
                </div>
              )}
