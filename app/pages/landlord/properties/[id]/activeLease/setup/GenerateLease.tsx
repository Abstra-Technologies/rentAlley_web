...
export default function GenerateLease({
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === "rent_amount" && value !== String(leaseDetails?.rent_amount)) {
      ...
      name === "rent_amount" &&
      value === String(leaseDetails?.rent_amount)
    ) {
  ...
  const handleVerifyOtp = async (e?: React.MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      const res = await axios.post(
        "/api/landlord/activeLease/veritfyOtp",
        payload
      );
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-5">
      {/* Help Button - Shows on all steps */}
      ...
      {step === 1 && (
        <>
          ...
          <div id="lease-type-selection" className="mb-6">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <button
                type="button"
                onClick={() =>
                  setForm((prev) => ({ ...prev, lease_type: "residential" }))
                }
              ...
              <button
                type="button"
                onClick={() =>
                  setForm((prev) => ({ ...prev, lease_type: "commercial" }))
                }
                className={`flex-1 px-4 py-3 rounded-lg border-2 transition-all text-sm font-semibold ${
            ...
            <p className="text-xs text-gray-500 mt-2">
              Residential = homes, condos, apartments. Commercial = offices,
              shops, business spaces.
            </p>
          </div>
          ...
          <div
            ...
            className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-sm mb-6 border-b pb-4"
          >
            {/* Tenant Info */}
            <div>
              <h3 className="text-gray-800 font-semibold mb-2 flex items-center gap-2">
                <Users className="w-4 h-4 text-emerald-600" /> Tenant
                Information
              </h3>
              <p>
              ...
              <p>
                ...
                {leaseDetails?.tenant_phone || "N/A"}
              </p>
              <p>
                <span className="font-medium text-gray-500">Address:</span>{" "}
                {leaseDetails?.tenant_address || "N/A"}
              </p>
              <p>
                <span className="font-medium text-gray-500">Citizenship:</span>{" "}
                {leaseDetails?.tenant_citizenship || "N/A"}
              </p>
              <p>
                <span className="font-medium text-gray-500">Civil Status:</span>{" "}
                {leaseDetails?.tenant_civil_status || "N/A"}
              </p>
              ...
              {leaseDetails?.tenant_birthdate && (
                <p>
                  ...
                  {Math.floor(
                    (new Date().getTime() -
                      new Date(leaseDetails.tenant_birthdate).getTime()) /
            ...
            <div>
              ...
              <p>
                <span className="font-medium text-gray-500">Name:</span>{" "}
                {leaseDetails?.landlord_name || "N/A"}
              </p>
              <p>
                <span className="font-medium text-gray-500">Email:</span>{" "}
                {leaseDetails?.landlord_email || "N/A"}
              </p>
              <p>
                <span className="font-medium text-gray-500">Phone:</span>{" "}
                {leaseDetails?.landlord_phone || "N/A"}
              </p>
              <p>
                <span className="font-medium text-gray-500">Address:</span>{" "}
                {leaseDetails?.landlord_address || "N/A"}
              </p>
              <p>
                <span className="font-medium text-gray-500">Citizenship:</span>{" "}
                {leaseDetails?.landlord_citizenship || "N/A"}
              </p>
              <p>
                <span className="font-medium text-gray-500">Civil Status:</span>{" "}
                {leaseDetails?.landlord_civil_status || "N/A"}
              </p>
              ...
              {leaseDetails?.landlord_birthdate && (
                <p>
                  ...
                  {Math.floor(
                    (new Date().getTime() -
                      new Date(leaseDetails.landlord_birthdate).getTime()) /
                      (1000 * 60 * 60 * 24 * 365.25)
                  )}{" "}
                  years old
                </p>
          ...
          {(rentChanged ||
            ...
            (leaseDetails?.end_date &&
              form.end_date &&
              form.end_date !== leaseDetails.end_date.split("T")[0])) && (
            <div
              id="change-warning"
              className="mb-5 flex items-start gap-2 bg-yellow-50 border border-yellow-300 text-yellow-700 p-3 rounded-lg text-sm"
            >
              <AlertTriangle className="w-4 h-4 flex-shrink-0 mt-0.5" />
              <span>
                You've modified one or more contract settings (rent, due date,
                grace period, late fee, or lease duration). These changes will
                also update the property, unit, and lease configurations
                accordingly.
              </span>
            </div>
          )}
          ...
          >
            ...
            <label className="flex flex-col">
              ...
              <input
                ...
                value={
                  form.start_date ||
                  leaseDetails?.start_date?.split("T")[0] ||
                  ""
                }
            ...
            <label className="flex flex-col">
              ...
              {rentChanged && (
                <span className="text-xs text-yellow-600 mt-1">
                  ⚠️ You modified the default rent. This will be saved to the
                  lease and update the unit record.
                </span>
              )}
            </label>
      ...
      {step === 2 && (
        <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-5 max-w-3xl mx-auto">
          ...
          <p className="text-sm text-gray-600 mb-6">
            These terms aren't stored in the database but will appear in the
            final lease document. Choose or confirm the most accurate options
            below.
          </p>

          <div
            id="additional-policies-form"
            className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm"
          >
            {/* Allowed Occupants */}
            <label id="occupants-notice" className="flex flex-col">
              <span className="block text-gray-600 mb-1 font-medium">
                Allowed Occupants
              </span>
              <select
            ...
            >
              ...
              >
                ...
                <option value="tenant_minor_landlord_major">
                  Tenant handles minor repairs; landlord handles major
                  maintenance
                </option>
                <option value="landlord_all">
                  Landlord responsible for all repairs and maintenance
                </option>
                <option value="tenant_all">
            ...
            >
              ...
              >
                <option value="">Select responsibility...</option>
                <option value="tenant">
                  Tenant pays for electricity and water
                </option>
            ...
            <label className="flex flex-col sm:col-span-2">
              ...
              >
                ...
                <option value="24hr_notice">
                  Landlord must provide at least 24-hour notice before entry
                ...
                <option value="emergency_entry">
                  Immediate entry allowed only in case of emergency or property
                  damage
                </option>
      ...
      {step === 3 && (
        <div
          id="review-section"
          className="bg-white border border-gray-200 rounded-2xl shadow-sm p-5 max-w-3xl mx-auto"
        >
          ...
          <div className="mb-5 flex items-start gap-2 bg-yellow-50 border border-yellow-300 text-yellow-700 p-3 rounded-lg text-sm">
            ...
            <span>
              ...
              <strong>Once confirmed, these details cannot be modified</strong>{" "}
              without creating a new lease record.
            </span>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-gray-700 mb-6">
            ...
            <p>
              ...
              {Number(form.rent_amount || 0).toLocaleString()}
            </p>
            <p>
              <strong>Security Deposit:</strong> ₱
              {Number(form.security_deposit || 0).toLocaleString()}
            </p>
            <p>
              <strong>Advance Payment:</strong> ₱
              {Number(form.advance_payment || 0).toLocaleString()}
            </p>

            <p>
              <strong>Billing Due Day:</strong> {form.billing_due_day || "—"}
            </p>
            <p>
              <strong>Grace Period:</strong> {form.grace_period_days || "—"}{" "}
              days
            </p>
            <p>
              <strong>Late Fee:</strong> ₱
              {Number(form.late_fee_amount || 0).toLocaleString()}
            </p>
            ...
            {rentChanged && (
              <div className="sm:col-span-2 flex items-start gap-2 bg-yellow-50 border border-yellow-200 text-yellow-700 p-3 rounded-md text-xs">
                ...
                <span>
                  The rent amount differs from the default unit rent. The new
                  rent will update both the unit and this lease record.
                </span>
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-gray-700 mb-6">
            ...
            <div className="bg-emerald-50/40 border border-emerald-100 rounded-lg p-4">
              ...
              <p>
                <strong>Name:</strong> {leaseDetails?.tenant_name || "N/A"}
              </p>
              <p>
                <strong>Email:</strong> {leaseDetails?.tenant_email || "N/A"}
              </p>
              <p>
                <strong>Phone:</strong> {leaseDetails?.tenant_phone || "N/A"}
              </p>
              <p>
              ...
              <p>
                ...
                {leaseDetails?.tenant_occupation || "N/A"}
              </p>
              <p>
            ...
            <div className="bg-blue-50/40 border border-blue-100 rounded-lg p-4">
              ...
              <p>
                <strong>Name:</strong> {leaseDetails?.landlord_name || "N/A"}
              </p>
              <p>
                <strong>Email:</strong> {leaseDetails?.landlord_email || "N/A"}
              </p>
              <p>
                <strong>Phone:</strong> {leaseDetails?.landlord_phone || "N/A"}
              </p>
              <p>
                <strong>Address:</strong>{" "}
                {leaseDetails?.landlord_address || "N/A"}
              ...
              <p>
                <strong>Occupation:</strong>{" "}
                {leaseDetails?.landlord_occupation || "N/A"}
              </p>
              <p>
                <strong>Civil Status:</strong>{" "}
                {leaseDetails?.landlord_civil_status || "N/A"}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm text-gray-700 mb-6">
            <p>
              <strong>Allowed Occupants:</strong>{" "}
              {form.allowed_occupants || "—"}
            </p>
            <p>
              <strong>Notice Period:</strong>{" "}
              {form.notice_period ? `${form.notice_period} days` : "—"}
            </p>
          ...
          <div
            ...
            className="flex items-start gap-2 bg-gray-50 border border-gray-200 p-3 rounded-lg text-sm text-gray-700 mb-6"
          >
            <input
              id="attest"
              type="checkbox"
            ...
            <label htmlFor="attest" className="cursor-pointer select-none">
              I hereby attest that all details provided above are true and
              correct to the best of my knowledge. I understand that once
              generated, this lease agreement becomes a binding digital record
              under the UpKyp platform.
            </label>
          </div>
      ...
      {step === 4 && (
        <div
          ...
          className="bg-white border border-gray-200 rounded-2xl shadow-sm p-5 max-w-2xl mx-auto text-center"
        >
          ...
          {!otpSent ? (
            <>
              <p className="text-sm text-gray-700 mb-5">
                To digitally sign and verify this lease, please authenticate
                using a one-time password (OTP) sent to{" "}
            ...
            <>
              <p className="text-sm text-gray-700 mb-4">
                ...
                <strong>{leaseDetails?.landlord_email}</strong>. Enter it below
                to confirm your digital signature.
              </p>
      ...
      {step === 5 && (
        <div className="text-center p-6 max-w-lg mx-auto">
          ...
          <p className="text-sm text-gray-600 mb-6">
            Your digital signature has been verified and recorded. You may now
            download or review your signed lease document.
          </p>
          <a
            href={leaseFileUrl}
            target="_blank"
