    ...
    return (
        <div className="min-h-screen bg-gray-50 pb-24 md:pb-6">
            <div className="w-full px-4 md:px-6 pt-20 md:pt-6">
                ...
                {configModal && (
                    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
                        <div className="bg-white w-11/12 max-w-md p-6 rounded-xl shadow-xl">
                            <h2 className="text-lg font-bold text-gray-900">Property Configuration Required</h2>
                            ...
                            <div className="flex justify-end mt-5 gap-2">
                                ...
                                <button
                                    ...
                                    onClick={() =>
                                        router.push(`/pages/landlord/properties/${property_id}/configurations?id=${property_id}`)
                                    }
                                >
                                    Go to Configuration
                                </button>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-4">
                        <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                                <h1 className="text-xl md:text-2xl font-bold text-gray-900">Property Billing</h1>
                                <p className="text-xs md:text-sm text-gray-600 mt-0.5">
                                    Manage property-level bills, generate invoices, and download summaries
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                            ...
                            propertyDetails?.electricity_billing_type === "submetered" ? (
                                <>
                                    ...
                                    <button
                                        disabled={configMissing}
                                        onClick={() => (configMissing ? Swal.fire("Configuration Required", "Please complete property configuration first.", "warning") : handleDownloadSummary())}
                                        className={`inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg font-semibold text-sm transition-all shadow-sm ${
                                            configMissing
                                                ? "bg-gray-300 text-gray-500 cursor-not-allowed"
                    ...
                    {configMissing && (
                        <div className="mb-4">
                            <div className="bg-red-50 border border-red-200 text-red-700 p-3 rounded-lg">
                                ⚠️ Billing features are disabled until the property configuration is completed.
                                <button
                                    onClick={() => router.push(`/pages/landlord/properties/${property_id}/configurations?id=${property_id}`)}
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mb-6 mt-4">
                        ...
                        <div className="p-4 bg-white rounded-xl shadow border border-gray-200">
                            ...
                            <p className="text-xl font-bold text-purple-600">
                                {bills.length === 0
                                    ? "0%"
                                    : Math.round((bills.filter((b: any) => b.billing_status !== "no_bill").length / bills.length) * 100) +
                                    "%"}
                            </p>
                        </div>
                    </div>
                                ...
                                <div className="flex items-start gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-emerald-700 mt-1">
                                            Monthly invoices are automatically generated for non-submetered units BY default.
                                        </p>
                                    </div>
                                </div>
                            </div>
                ...
                <UnitMeterReadingsModal isOpen={openMeterList} onClose={() => setOpenMeterList(false)} property_id={property_id} />
                ...
                <div className="block md:hidden space-y-3 mb-6">
                    {loadingBills ? (
                        <div className="animate-pulse space-y-3">
                            {[...Array(3)].map((_, i) => (
                                <div key={i} className="bg-gray-100 rounded-lg h-32 w-full"></div>
                            ))}
                        </div>
                    ) : bills.length === 0 ? (
                        <div className="bg-white rounded-lg border-2 border-dashed border-gray-300 p-8 text-center">
                            ...
                            <p className="text-gray-900 font-semibold text-base mb-1">No Active Units</p>
                            <p className="text-gray-500 text-sm">No active units found for billing this month</p>
                        </div>
                    ) : (
                        bills.map((bill: any) => (
                            <div key={bill.unit_id} className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                                <div className="p-4">
                                    <div className="flex items-start justify-between mb-3">
                                        <div className="flex items-center gap-2 flex-1 min-w-0">
                                            <Building2 className="w-5 h-5 text-blue-600 flex-shrink-0" />
                                            <div className="min-w-0 flex-1">
                                                <h3 className="font-bold text-gray-900 text-base truncate">{bill.unit_name}</h3>
                                                <p className="text-xs text-gray-500 truncate">{bill.tenant_name}</p>
                                            </div>
                                        </div>
                                        <span
                      ...
                      {bill.billing_status || "no_bill"}
                        ...
                        {bill.billing_period ? new Date(bill.billing_period).toLocaleDateString() : "—"}
                      </span>
                                        </div>
                                        <div className="flex justify-between text-sm">
                                            <span className="text-gray-600">Amount:</span>
                                            <span className="font-bold text-gray-900">
                        ₱{Number(bill.total_amount_due).toLocaleString("en-PH", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </span>
                                        </div>
                                    </div>
                                        ...
                                        <button
                                            onClick={() =>
                                                guardActionWithConfig(() =>
                                                    router.push(`/pages/landlord/properties/${property_id}/billing/createUnitBill/${bill.unit_id}`)
                                                )
                                            }
                                    ...
                                    {propertyDetails?.water_billing_type !== "submetered" &&
                                        propertyDetails?.electricity_billing_type !== "submetered" && (
                                            ...
                                            >
                                                <FileText className="w-4 h-4" />
                                                {bill?.billing_status === "no_bill" ? "Generate Bill" : "Review Bill"}
                                            </button>
                                        )}
                                </div>
                            </div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-200">
                            ...
                            {loadingBills ? (
                                ...
                                <tr>
                                    <td colSpan={6} className="px-4 py-12">
                                        <div className="text-center">
                                            ...
                                            <p className="text-gray-900 font-semibold text-lg mb-1">No Active Units</p>
                                            <p className="text-gray-500 text-sm">No active units found for billing this month</p>
                                        </div>
                                    </td>
                                </tr>
                                ...
                                bills.map((bill: any) => (
                                    <tr key={bill.unit_id} className="hover:bg-gray-50 transition-colors">
                                        <td className="px-4 py-3">
                                            <div className="flex items-center gap-2">
                                                <Building2 className="w-4 h-4 text-blue-600" />
                                                <span className="font-semibold text-gray-900 text-sm">{bill.unit_name}</span>
                                            </div>
                                        </td>
                                        <td className="px-4 py-3 text-sm text-gray-700">{bill.tenant_name}</td>
                                        <td className="px-4 py-3 text-sm text-gray-600">{bill.billing_period ? new Date(bill.billing_period).toLocaleDateString() : "—"}</td>
                                        <td className="px-4 py-3 text-sm font-bold text-gray-900">₱{Number(bill.total_amount_due).toLocaleString("en-PH", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                                        <td className="px-4 py-3">
                        <span className={`inline-flex items-center px-2.5 py-1 text-xs font-semibold rounded-full border ${getStatusConfig(bill.billing_status)}`}>
                          {bill.billing_status || "no_bill"}
                        </span>
                                        </td>
                                        <td className="px-4 py-3">
                                            <div className="flex items-center justify-center gap-2">
                                                {(propertyDetails?.water_billing_type === "submetered" || propertyDetails?.electricity_billing_type === "submetered") && (
                                                    <button
                                                        onClick={() =>
                                                            guardActionWithConfig(() =>
                                                                router.push(`/pages/landlord/properties/${property_id}/billing/createUnitBill/${bill.unit_id}`)
                                                    ...
                                                    >
                                                        {bill.billing_status === "draft" ? "Edit Bill" : bill.billing_status === "no_bill" ? "Generate Bill" : "Review Bill"}
                                                    </button>
                                                )}
