...
export default function CreateSubmeteredUnitBill() {
  ...
  const [propertyRates, setPropertyRates] = useState({ waterRate: 0, electricityRate: 0 });
...
const today = new Date().toISOString().split("T")[0];
  ...
  async function fetchPDCData(billingId?: number, leaseId?: number) {
    try {
      ...
      if (Array.isArray(data?.pdcs)) {
        _pdc = data.pdcs.find((x: any) => x.status === "pending") ||
  ...
  async function fetchUnitData() {
    try {
      const res = await axios.get(`/api/landlord/billing/submetered/getUnitBilling?unit_id=${unit_id}`);
      const data = res.data;
      if (!data.unit || !data.property) throw new Error("Missing unit or property data.");
      setUnit(data.unit);
      setProperty(data.property);
      const dueDate = data.dueDate ? new Date(data.dueDate).toISOString().split("T")[0] : "";

      const rateRes = await axios.get(`/api/landlord/billing/checkPropertyBillingStats?property_id=${data.property.property_id}`);
      const billing = rateRes.data.billingData;

      setPropertyRates({
        waterRate: billing?.water?.total && billing?.water?.consumption ? billing.water.total / billing.water.consumption : 0,
        electricityRate: billing?.electricity?.total && billing?.electricity?.consumption ? billing.electricity.total / billing.electricity.consumption : 0,
      });

      const eb = data.existingBilling;
      setForm((prev) => ({
        ...prev,
        readingDate: eb?.reading_date ? new Date(eb.reading_date).toISOString().split("T")[0] : new Date().toISOString().split("T")[0],
        dueDate: eb?.due_date ? new Date(eb.due_date).toISOString().split("T")[0] : dueDate,
      ...
      setExtraExpenses(eb?.additional_charges?.map((c: any) => ({ charge_id: c.id, type: c.charge_type, amount: c.amount, fromDB: !!eb.billing_id })) || []);
      setDiscounts(eb?.discounts?.map((d: any) => ({ charge_id: d.id, type: d.charge_type, amount: d.amount, fromDB: !!eb.billing_id })) || []);
  ...
  const handleAddExpense = () => setExtraExpenses([...extraExpenses, { type: "", amount: 0 }]);
  ...
  const handleRemoveExpense = async (index: number, item: any) => {
    try {
      if (item.fromDB && item.charge_id) {
        const confirm = await Swal.fire({ title: "Delete this additional charge?", text: "This will permanently remove it from the billing record.", icon: "warning", showCancelButton: true, confirmButtonText: "Yes, delete it" });
        if (!confirm.isConfirmed) return;
        await axios.delete("/api/billing/non_submetered/deleteCharge", { data: { charge_id: item.charge_id } });
  ...
  const handleAddDiscount = () => setDiscounts([...discounts, { type: "", amount: 0 }]);
  ...
  const handleRemoveDiscount = async (index: number, item: any) => {
    try {
      if (item.fromDB && item.charge_id) {
        const confirm = await Swal.fire({ title: "Delete this discount?", text: "This will permanently remove it from the billing record.", icon: "warning", showCancelButton: true, confirmButtonText: "Yes, delete it" });
        if (!confirm.isConfirmed) return;
        await axios.delete("/api/billing/non_submetered/deleteCharge", { data: { charge_id: item.charge_id } });
  ...
  const calculateBill = () => {
    ...
    const totalExtraCharges = extraExpenses.reduce((sum, item) => sum + (parseFloat(String(item.amount)) || 0), 0);
    const totalDiscounts = discounts.reduce((sum, item) => sum + (parseFloat(String(item.amount)) || 0), 0);
    const totalBeforePdc = rent + dues + waterCost + elecCost + totalExtraCharges - totalDiscounts;
    ...
    const adjustedTotal = rentAfterPdc + dues + waterCost + elecCost + totalExtraCharges - totalDiscounts;
    return {
      waterUsage, elecUsage, waterCost, elecCost,
      rent, dues, lateFee, totalExtraCharges, totalDiscounts,
      totalBeforePdc, adjustedTotal, pdcAmount, pdcCleared, pdcCoveredAmount
    };
  };

  const bill = useMemo(() => {
    if (!unit || !property) return {
      rent: 0, dues: 0, waterUsage: 0, elecUsage: 0, waterCost: 0, elecCost: 0,
      totalExtraCharges: 0, totalDiscounts: 0, baseTotal: 0, adjustedTotal: 0, pdcAmount: 0, pdcCleared: false
    };
  ...
  const handleMarkCleared = async () => {
    ...
    const confirm = await Swal.fire({ title: "Mark PDC as Cleared?", text: "This will mark the check as cleared and apply it against this billing.", icon: "question", showCancelButton: true, confirmButtonText: "Yes, mark cleared" });
    if (!confirm.isConfirmed) return;
    try {
      const res = await axios.put(`/api/landlord/pdc/updateStatus`, { pdc_id: pdc.pdc_id, status: "cleared" });
      if (res.status === 200) {
  ...
  try {
    ...
    // ---- FINAL PAYLOAD (aligned with new API) ----
    const payload = {
      unit_id: unit.unit_id,

      // NEW
      billingDate: form.billingDate, // <- ADDED
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24 md:pb-6">
      <div className="w-full px-4 md:px-6 pt-20 md:pt-6">
        {property && <BackButton label="Back to Units" fallback={`/pages/landlord/properties/${property.property_id}/billing`} />}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr] gap-6 lg:gap-8">
            ...
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <DatesForm form={form} setForm={setForm} hasExistingBilling={hasExistingBilling} />
              </div>

              <MeterReadingForm form={form} setForm={setForm} property={property} existingBillingMeta={existingBillingMeta} />

              <ExtraChargesForm extraExpenses={extraExpenses} handleAddExpense={handleAddExpense} handleExpenseChange={handleExpenseChange} handleRemoveExpense={handleRemoveExpense} />

              <DiscountsForm discounts={discounts} handleAddDiscount={handleAddDiscount} handleDiscountChange={handleDiscountChange} handleRemoveDiscount={handleRemoveDiscount} />

              {/* Mobile submit */}
              <div className="mt-8 block lg:hidden">
                <button onClick={handleSubmit} className={`w-full px-6 py-3 rounded-lg font-semibold shadow-sm ${hasExistingBilling ? "bg-amber-500 hover:bg-amber-600 text-white" : "bg-emerald-600 hover:bg-emerald-700 text-white"}`}>
            ...
            <div className="space-y-5 lg:sticky lg:top-4 lg:h-fit">
              <BillingSummary bill={bill} unit={unit} property={property} extraExpenses={extraExpenses} discounts={discounts} pdc={pdc} loadingPdc={loadingPdc} />

              <PDCCard pdc={pdc} loadingPdc={loadingPdc} handleMarkCleared={handleMarkCleared} />

              <ActionsBar onSubmit={handleSubmit} hasExistingBilling={hasExistingBilling} />
            </div>
          </div>
        </div>
      </div>

      <PropertyRatesModal isOpen={isRateModalOpen} onClose={() => setIsRateModalOpen(false)} billingData={null} billingForm={{ billingPeriod: "", electricityConsumption: "", electricityTotal: "", waterConsumption: "", waterTotal: "" }} propertyDetails={property} hasBillingForMonth={false} handleInputChange={() => {}} handleSaveOrUpdateBilling={() => {}} />
    </div>
  );
}
