  ...
  const detectCameraDevices = async () => {
    try {
      if (!navigator.mediaDevices || !navigator.mediaDevices.enumerateDevices) {
        setCameraError("Camera detection is not supported in this browser. Please use a modern browser like Chrome, Firefox, or Safari.");
        setHasCamera(false);
      ...
      if (!hasVideoInput) {
        setCameraAllowed(false);
        setCameraError("No camera detected. Please connect a camera or use a device with a camera.");
      }
    } catch (err) {
      console.error("Device enumeration failed:", err);
      setCameraError("Failed to detect camera devices. Please check your browser settings or try a different browser.");
      setHasCamera(false);
      setCameraAllowed(false);
    }
  };
  ...
  const requestCameraPermission = async () => {
    try {
      ...
      if (window.location.protocol !== "https:") {
        setCameraError("Camera access requires a secure connection (HTTPS). Please access this page over HTTPS.");
        setCameraAllowed(false);
        return;
      }
      ...
      const stream = await navigator.mediaDevices.getUserMedia({
        video: { facingMode: { ideal: "user" } }, // Use ideal for mobile compatibility
        audio: false,
      });
      ...
      switch (err.name) {
        case "NotAllowedError":
          setCameraError("Camera access denied. Please enable camera permissions in your browser or device settings and refresh the page.");
          break;
        case "NotFoundError":
          setCameraError("No camera found. Please connect a camera or use a device with a camera.");
          break;
        case "NotReadableError":
          setCameraError("Camera is in use by another application. Please close other apps using the camera and try again.");
          break;
        case "SecurityError":
          setCameraError("Camera access requires a secure connection (HTTPS). Please ensure the site is loaded over HTTPS.");
          break;
        default:
          setCameraError("Unable to access the camera. Please check your browser and device settings.");
      }
    }
  };

  // 🧠 Auto-check whenever camera opens
  useEffect(() => {
  ...
  const handleEnhancedCapture = async () => {
    if (webcamRef.current) {
      ...
      try {
        ...
        if (quality.isBlurry) {
          setCaptureGuidance("Image is blurry. Please hold steady and try again.");
        ...
        if (quality.isTooLight) {
          setCaptureGuidance("Image is overexposed. Adjust lighting and try again.");
          setIsAnalyzing(false);
          return;
        }
                            ...
                            <div className="space-y-2">
                                ...
                                {phoneNumber && !/^09\d{9}$/.test(phoneNumber) && (
                                    <p className="text-red-500 text-xs mt-1">
                                        Phone number must be 11 digits and start with 09 (e.g., 09213218888)
                                    </p>
                                )}
                            </div>
                              ...
                              <div className="mt-6 space-y-4">
                                ...
                                <div className="flex gap-3">
                                  <button
                                      ...
                                      disabled={isAnalyzing || !cameraAllowed}
                                  ...
                                  <button
                                      ...
                                      disabled={isAnalyzing || captureCountdown > 0 || !cameraAllowed}
                        ...
                        <div className="space-y-4">
                          {cameraError ? (
                              ...
                              <div className="text-center p-6 bg-red-50 border border-red-200 rounded-xl">
                                ...
                                <h3 className="text-red-700 font-semibold mb-2">No Camera Detected</h3>
                                <p className="text-sm text-red-600 mb-4">
                                  Please connect a webcam or enable your mobile camera.
                                </p>
                                <button
                                    onClick={detectCameraDevices}
                                    className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-medium transition-all duration-200"
                      ...
                      <div className="grid gap-4 md:grid-cols-2">
                        ...
                        <div className="bg-white p-4 rounded-lg border">
                          ...
                          <p className="text-gray-900">
                            {DOCUMENT_TYPES.find((doc) => doc.value === selectedDocument)?.label}
                          </p>
                        </div>
                      </div>
                    </div>
