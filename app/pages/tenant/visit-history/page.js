...
const PropertyVisits = () => {
  ...
  const handleMarkAttendance = async (visitId, attended) => {
    try {
      await axios.put("/api/tenant/property-finder/viewBookings/markAttendance", {
        visit_id: visitId,
        attended: attended
      });
  ...
  const getStatusBadge = (status, attended) => {
    switch (status?.toLowerCase()) {
      ...
      case "approved":
        return <span className="bg-green-100 text-green-700 px-3 py-1 rounded-full text-sm font-medium">✓ Confirmed</span>;
      case "cancelled":
        return <span className="bg-gray-100 text-gray-600 px-3 py-1 rounded-full text-sm font-medium">Cancelled</span>;
      case "completed":
        if (attended === true) {
          return <span className="bg-emerald-100 text-emerald-700 px-3 py-1 rounded-full text-sm font-medium">✓ Attended</span>;
        } else if (attended === false) {
          return <span className="bg-red-100 text-red-700 px-3 py-1 rounded-full text-sm font-medium">✗ No Show</span>;
        } else {
          return <span className="bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-sm font-medium">Completed</span>;
        }
      default:
        return <span className="bg-gray-100 text-gray-600 px-3 py-1 rounded-full text-sm font-medium">{status}</span>;
    }
  };
        ...
        <div className="px-8 py-6">
          {loading ? (
            ...
            <div className="space-y-6">
              {/* Summary card matching your style */}
              {upcomingVisits.length > 0 && (
                <div className="bg-blue-50 border border-blue-100 rounded-lg p-6">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-blue-500 rounded-lg flex items-center justify-center">
                      <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3a1 1 0 011-1h6a1 1 0 011 1v4m-8 0h8m-8 0V6a1 1 0 00-1 1v9a1 1 0 001 1h8a1 1 0 001-1V7a1 1 0 00-1-1" />
                      </svg>
                    </div>
                    <div>
                      <h2 className="text-lg font-semibold text-gray-900">Upcoming Visits</h2>
                      <p className="text-gray-600">You have {upcomingVisits.length} upcoming property {upcomingVisits.length === 1 ? 'visit' : 'visits'}</p>
                    </div>
                  </div>
                </div>
              )}
                      ...
                      >
                        <div className="flex items-start justify-between mb-4">
                          <div className="flex-1">
                            ...
                            <p className="text-gray-600 mb-3">{visit?.unit_name}</p>
                          </div>
                        ...
                        {visit?.disapproval_reason && (
                          <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                            <p className="text-sm text-yellow-800">
                              <span className="font-medium">Note:</span> {visit?.disapproval_reason}
                            </p>
                          </div>
                        )}

                        {visit?.status === "completed" && visit?.attended === undefined && (
                          <div className="flex justify-end gap-3 pt-3 border-t border-gray-100">
                            <button
                              onClick={() => handleMarkAttendance(visit?.visit_id, false)}
                              className="px-3 py-2 text-red-600 border border-red-300 rounded-lg hover:bg-red-50 transition-colors text-sm font-medium"
                            >
                              Mark No Show
                            </button>
                            <button
                              onClick={() => handleMarkAttendance(visit?.visit_id, true)}
                  ...
                  <details className="group">
                    <summary className="cursor-pointer list-none mb-4">
                      <div className="flex items-center gap-2 text-gray-600 hover:text-gray-900 transition-colors">
                        ...
                        <span className="font-medium">Past Visits ({pastVisits.length})</span>
                      </div>
                    </summary>
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <div className="flex-1">
                              ...
                              <p className="text-sm text-gray-600 mb-2">{visit?.unit_name}</p>
