...
export async function GET(req: Request) {
    try {
        ...
        const qHashed = crypto.createHash("sha256").update(qLower).digest("hex");

        let rows: any[] = [];

        if (isEmail) {
            // 🔍 Search by hashed email
            ...
            // 🔍 Search by nameHashed (full name) or nameTokens (partial)
            [rows] = await db.query(
                `
                    SELECT
                        la.agreement_id,
        ...
        // 🧠 Safely decrypt sensitive fields
