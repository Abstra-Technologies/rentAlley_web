...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const tenant_id = (prospectiveTenant as any[])[0].tenant_id;
    const valid_id_encrypted = (prospectiveTenant as any[])[0].valid_id;
    const decryptedValidId = decryptData(
      JSON.parse(valid_id_encrypted),
      encryptionSecret!
    );
