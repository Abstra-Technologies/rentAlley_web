...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const values = [
      ...
      rentAmt,
      furnish,
      requires_security_deposit ? 1 : 0,
      requires_advance_payment ? 1 : 0,
      amenityString,
      status || "unoccupied",
    ];
    ...
    const [result] = await connection.execute(
        `INSERT INTO Unit
         (property_id, unit_name, unit_size, bed_spacing, avail_beds, rent_amount, furnish, requires_security_deposit, requires_advance_payment, amenities, status)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        values
    );
