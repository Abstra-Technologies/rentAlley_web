...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        if (rows.length === 0 || !rows[0].google_id) {
            return redirectToLogin("User not registered with Google. Use email/password to login.");
        }
        ...
        response.cookies.set("token", token, { path: "/", httpOnly: true, sameSite: "lax" });
...
async function sendOtpEmail(email: string, otp: number) {
    ...
    try {
        ...
        console.error("[Google OAuth] Failed to send OTP:", err);
    }
}
